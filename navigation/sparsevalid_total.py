    ...
    def test(self, options, n_epochs: int = 1, n_steps=10, save=False):
        ...
        for epoch_idx in range(n_epochs):
            for step_idx in range(5):
                ...
                print('Percentile {}: Step: {}. Loss: {}. Err: {}cm, Sparsity: {:3f}.'.format(
                        self.percentile, step_idx,
                        np.round(loss, 3), np.round(100 * err, 2), sparsity))
                with open(self.save_repo + self.savefile + '.txt', 'a') as the_file:
                    the_file.write('Percentile {}: Step: {}. Loss: {}. Err: {}cm. Sparsity: {:.2f}\n'.format(self.percentile, step_idx, np.round(loss, 3), np.round(100 * err, 2), sparsity))
